...
function ShaderCanvas({ fragSource, pixelRatio }: ShaderCanvasProps) {
...
interface BlueprintShaderGridProps {
  height?: string;
  style?: React.CSSProperties;
  children?: React.ReactNode;
}

export default function BlueprintShaderGrid({
  height = "auto",
  style,
  children,
}: BlueprintShaderGridProps) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const [shouldRender, setShouldRender] = React.useState(false);

  useEffect(() => {
    // Mobile check & concurrency check for auto-disable
  ...
  return (
    <div
      ...
      style={{
        ...
        height: height,
        overflow: "hidden",
        backgroundColor: "#0d1829", // Fallback / matching color
        ...style,
      }}
    >
      {shouldRender && <ShaderCanvas fragSource={SHADER_SRC} />}
      {/* Container for content on top of shader */}
